...
1. **观众理解检查**：首次观看者脱离项目上下文，能否第一眼看懂唯一信息点；删除阶段编号、任务状态、内部路径和工作备注。
2. **画面独立合规检查**：脱离口播单看画面，检查工具/Logo 墙、下载安装暗示、二维码/外链、互动换利益、夸大承诺、敏感信息、第三方 UI、无效水印、马赛克残留和贯穿式品牌小字。

两项任一有疑虑，立即退回重做，不得进入 10 秒样片、完整成片或发布。机器预审只提供风险线索，不得把相关性写成平台已确认规则；若预审仍失败，先取得命中范围或时间点再定点修改，不盲删素材。
...
永不：跳过确认门 / 覆盖旧版本 / 让AI画角标 / 照抄抽检响度参数 / 索取或读取API Key / 只审口播不审最终画面 / 堆叠工具名称墙或下载式特效 / 凭模糊预审提示盲删素材
```
